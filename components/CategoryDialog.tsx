...
export default function CategoryDialog({onCategory, category}: CategoryDialogProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="m-0 max-h-screen max-w-max overflow-y-scroll p-0">
        ...
        />
        <CategoryFooter onCategory={handleApplyFilter} />
      </DialogContent>
    </Dialog>
  );
}

function CategoryFooter({onCategory}: {onCategory: () => void}) {
  return (
    <div className="grid grid-cols-2 gap-3 p-5">
      ...
      <Button variant="default" className="rounded-full" onClick={onCategory}>
        Apply Filter
      </Button>
    </div>
  );
